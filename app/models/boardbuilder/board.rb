class Boardbuilder::Board < ApplicationRecord
  ...
  has_many :cells, inverse_of: :board, foreign_key: 'boardbuilder_board_id', dependent: :delete_all
  ...
  validates :columns, presence: true, numericality: { only_integer: true, greater_than: 0, less_than_or_equal_to: 50 }
  ...
  def populate_cells
    ...
    if cells.count < required_cells
      to_create = []
      current_cells = cells.count
      (required_cells - current_cells).times do |index|
        new_cell = cells.new.attributes
        new_cell[:index] = current_cells + index + 1
        to_create << new_cell
      end
      if to_create.length > 0
        Boardbuilder::Cell.insert_all(to_create)
      end
